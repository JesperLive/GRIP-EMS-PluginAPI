...
v2 added owner-scoped writes: a plugin can author its own sequences, override settings, and request a CVar profile. Those go through validated, journaled paths that reach only the plugin's own objects — they don't open anything on this list. Authoring your own sequence isn't a write into rotation execution, and requesting a CVar profile isn't a secure CVar write. The boundary moved out to include owned, reversible writes; the locked core didn't move.
...
| Persistence | A plugin writes only through validated, owner-scoped paths — its own sequences and setting overrides. Raw writes to the saved-variables tables or the profile database risk silent corruption, so there's no direct write path. |
